...
const source = fs.readFileSync('F:/aaa/final/tmp-workcases-hard-source.txt', 'utf8')
...
const parts = source.split(/## SERVICE (\d+)：/)
const groupsJa = []
...
for (let i = 1; i < parts.length; i += 2) {
  ...
  const rawTitle = rest.slice(0, titleEnd).trim()
  const serviceTitle = rawTitle.replace(/（高難易度）$/, '')
  const block = rest.slice(titleEnd)

  const caseMatch = block.match(/### 事例：(.+?)\n\n([\s\S]+?)(?=\n\n---|\n\n##|$)/)
  if (!caseMatch) {
    throw new Error(`Failed to parse case for SERVICE ${num}`)
  }

  const title = caseMatch[1].trim()
  const body = caseMatch[2]
  const challenge = parseSection(body, '課題').replace(/\n- /g, '\n').replace(/^- /, '').trim()
  const technicalDifficulty = parseBullets(parseSection(body, '技術的難易度'))
  const solution = parseBullets(parseSection(body, '解決策')) || parseSection(body, '解決策')
  const result = parseBullets(parseSection(body, '結果')) || parseSection(body, '結果')

  groupsJa.push({
    serviceNumber: num.padStart(2, '0'),
    serviceTitle,
    cases: [
      {
        id: `${num.padStart(2, '0')}-1`,
        title,
        challenge,
        technicalDifficulty,
        solution,
...
function renderGroups(groups, locale) {
  const lines = groups.map((group) => {
    const title = locale === 'ja' ? group.serviceTitle : serviceTitlesEn[group.serviceNumber]
    const caseLines = group.cases
      .map((c) => {
        ...
        technicalDifficulty: '${escapeTs(data.technicalDifficulty)}',
...
if (groupsJa.length !== 17) {
  console.error(`Expected 17 groups, got ${groupsJa.length}`)
  process.exit(1)
}
